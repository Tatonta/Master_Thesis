...
class TBSaver(object):
    ...
    def __init__(self, base_output_dir: Path, title: str, args: dict, sub_dirs=('train', 'test')):
        # Create experiment directory
        timestamp_str = datetime.fromtimestamp(
            time()).strftime('%Y-%m-%d_%H-%M-%S')
        self.path = base_output_dir / f'{timestamp_str}_{title}'
    ...
    def save(self, net: torch.nn.Module, name: str, epoch: int):
        """
        Save model parameters in the checkpoint directory.
        """
        torch.save(net.state_dict(), self.ckpt_path /
                   f'{name}_{epoch:05d}.pth')
