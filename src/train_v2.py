...
from algorithms.PPOv3 import PPO
from utils.config import *
from utils.saver import TBSaver

# server = ('192.168.1.59', 5000)
#
...
def train():
    # setup logger
    logger = TBSaver(base_output_dir=exp_path, title="PPO-Glacier", args={})
    ...
    call_args = {"Difficulty": difficulty ,
                 "LevelName": "Glacier" , "FullReset": True}
    ...
    while i_episode <= max_training_timesteps:
        ...
        state = env.reset(
            options={"Difficulty": difficulty, "LevelName": "Glacier", "SimulationSpeed": 4.0})
        ...
        for t in range(1, 500):
            ...
            compass_state_np = np.array([state["sin"], state["cos"], state["distance"]])
            compass_state = torch.from_numpy(compass_state_np).view(-1).unsqueeze(0).to(device)

            state = {
                'image': img_feat,
                'compass': compass_state
            }
            action, prob, val = ppo_agent.select_action(state)
            # json_mess = json.dumps(action).encode()
            # s.sendto(json_mess, server)
            new_state, reward, done, _ = env.step(action)

            # saving reward and is_terminals
            ppo_agent.store(img_feat, compass_state, action, prob, val, reward, done)
